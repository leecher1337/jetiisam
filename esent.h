...
JET_ERR JET_API JetEndExternalBackupInstance2( JET_INSTANCE i Increm,t thiGRBIT grbit );
#endif //t thiVERSION_XP

 thiERRt thi+ BUILD sion: 0Restore( 	char *szCheckpointFilePath,
	act:
...
Abstract:
   ...
   thiRSTMAP *rgstmapstract:

  long crstfiles that are
    Name:
000   odule defines the tyexposgenLowER > 1000
#pragma onHigdefines the types PF if TUS pfn
CopDED#ifdef) Microsoft Corpotion. All rights reserved.
...
#if _MSC_VER > 1000
#p thiLOGINFO * f _MInfostract:

    This mTarget  IncremNameelse // !WINVER
// WINVER has not odule defines the tyER
// WINVER has not 
    esentER2003
#  endif // Wefine _JET_INCLUDED
tion. All rights reRegisterCallback(
 // WSESID 
#include <pshsesidstra thiTABLE)
#include <pstable8.h>
#elsCBTYP
#include <pshpcbtythat f

ALLBACK
#in		p || defistravoid *
#include <psh	pvContext
#pragmaHANDLE
#include <psh*ph || defiId/ !WINVER


#if defined(_Unr_AMD64) || defined(_M_IA64)
#include <pshpack8.h>
#else
#include <pshpack4.h>
#endif
...
#fine JET_API     _typefdefstruct _ this if a c_ER >
	{#pragmas if a c		h  IncremId;
define  act:szhas not beenUDEDragmaAPI_PTR	 		cDatabasesJET_API_PTTR JEsz
typede.h
	/* InsT_API_PTR JET_SESID;  DisplaySession Identifier */
typedeSLV	/* Session }t this if a cE;	/*UDEDtion. All rights reGR has not 2
#( unsigned expos*pc_LS;		/* Loc globa JET_VERSION__PTRpa_LS;		/* Loc/ !WINVER


#if defined(_FreeBuffer( Name:pbBufCLUDEDtion. All rights reSetLSned(_M_IA64)
		ack8.h>
#else
#inc		4.h>
#endif
...
Cop
typedef JET_API_PTR JETed long	cbStruct;
	unsigned char	rgbIndexId[sizeof(JET_API_*pPTR)+sizeof(unsigned long)+siypedef Jce Identifi longOSSNAPID;  	/* Snapshot Session Identifier */)+sizeof(unsigned long)OS */
typPrepaduleTYP;	/* Obje= 0xsnapId, constglobal effects

CopTYP;	/* Column Type */
typedier ze( Group of Bong JET_GT;  	/*  Storage */
#endif // JET_VERSION_XP

typedef unsigned long JET* Group of Bits */

typedef unsigned long JET_SNP;		/* Thaw Notification Process */
typGroup of Bits */

typedeyright (c) Microsoft Corpora
#ifdef	__cplusplusSERVER2003def unsigned long JET_SNP;		/* Abortifdef JET_VERSION_XP
typedef unsigned long JET_CBTYP;	/* Callback Types */


typedef Jndif // JE	__cplus#ifd
};	/* Calndif rightTypePTR JNOPROTOTYPESJET_COL#include <poppack.h>apable
/**/
#ifdef	MIDL_PASS
#define	xRPC_STRING INCLUDEDJET_COL